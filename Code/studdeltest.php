 ...
 <div class="container">
  <div class="row">
    ...
    <div class="col-md-6">
                   ...
                   <div class="form-group">
                    ...
                    <select name="txtdelsubmission" class="form-control" required>
                        ...
                        <?php
                          ...
                          $sql="SELECT testid,testname,testdate,teststarttime,testendtime,tblSubmtest.test_answr_id,tblSubmtest.test_answr from tblTest INNER JOIN tblSubmtest ON tblSubmtest.subm_testid=tblTest.testid where testgid='$grpr2[0]' AND  tblSubmtest.submtest_gstudid='$gstudrow[0]' AND  tblSubmtest.test_answr_status=1 AND teststatus=1";
      ...
      echo '<option value="'.$row['test_answr_id'].'|'.$row['testid'].'">'.$row['test_answr'].' --- '.$row['testname'].' &nbsp &nbsp '.$day.'-'.$month3.'-'.$year.'   ( '.date("g:i a",strtotime($row['teststarttime'])).' -- '.date("g:i a",strtotime($row['testendtime'])).' ) </option>';
                          }
...
$sql1row=mysqli_fetch_array($sql1result);
  ...
  $sql2="UPDATE tblSubmtest set test_answr_status=-1 where submtest_gid='$grpr2[0]' AND submtest_gstudid='$sql1row[0]' AND subm_testid='$explode_result[1]' AND test_answr_status=1";
