...
<body>
  ...
  <div class="myflexcontainer">
        ...
        $uid=$_SESSION['userid'];
         ...
         $uid=$_SESSION['userid'];
         $sql="SELECT tblGroup.gname,tblSubject.subjname,tblCourse.cname from tblStudent INNER JOIN tblGroupstud ON tblGroupstud.gstudemail=tblStudent.studemail INNER JOIN tblSubject ON tblSubject.subjcid=tblStudent.studcid AND tblSubject.subjsem=tblStudent.studsem INNER JOIN tblCourse ON tblCourse.cid=tblStudent.studcid INNER JOIN tblGroup ON tblGroup.gsubjid=tblSubject.subjid AND tblGroupstud.sgid=tblGroup.gid where tblGroupstud.gstudemail='$uid' AND tblGroupstud.gstudstatus=1 AND tblGroup.gstatus=1 AND tblSubject.subjstatus=1 AND gstudemail in (select uname from tblLogin where status=1)";
         ...
         while($r=mysqli_fetch_array($result))
         {
          echo '<div class="item">';
          echo  '<h1>';
           echo '<a href="studinsidegrp.php?gname='.$r['gname'].'">'.$r['gname'].'</a>';
          echo '</h1>';
