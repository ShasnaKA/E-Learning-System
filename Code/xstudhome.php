...
.item{
  ...
  margin-bottom:20px;
  padding:18px;
  background-color:#159675 ;//#0986; //#b41f4e   ;
  //flex-basis:50%;
  box-sizing:border-box;
...
<body>
  ...
  <div class="myflexcontainer">
        ...
        $uid=$_SESSION['userid'];
         ...
         $sql="SELECT tblXGroup.xgname,tblXTutor.xtutorname from tblXStudent INNER JOIN tblXGroupstud ON tblXGroupstud.xgstudemail=tblXStudent.xstudemail  INNER JOIN tblXGroup ON tblXGroup.xgid=tblXGroupstud.xsgid INNER JOIN tblXTutor ON tblXGroup.xtutorhostemail=tblXTutor.xtutoremail where tblXGroupstud.xgstudemail='$uid' AND tblXGroupstud.xgstudstatus=1 AND tblXGroup.xgstatus=1 AND xgstudemail IN(SELECT uname from tblLogin where status=1)";
