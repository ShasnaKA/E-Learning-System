...
include "xstudinsidegrpcommon.php";
...
$qry="SELECT DATE_FORMAT(xtestdate,'%Y,%m,%d'),CONCAT(CONCAT(CONCAT(CONCAT(xtestname,'<br>'),DATE_FORMAT(xteststarttime,'%I %p'),' -- '),DATE_FORMAT(xtestendtime,'%I %p'))) from tblXTest where xtestgid='$gid' and xteststatus=1 AND CONCAT(CONCAT(xtestdate,' '),xteststarttime)>='$cdateandtime'";
//$res=mysql_query($qry);
$res=$conn->query($qry);
$a=array();
