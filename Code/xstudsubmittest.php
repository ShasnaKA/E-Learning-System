 ...
 <div class="container">
  <div class="row">
    ...
    <div class="col-md-6">
                   ...
                   <div class="form-group">
                    ...
                    <select name="txtsubmtestname" class="form-control" required>
                        ...
                        <?php
                          ...
                          $sql="SELECT xtestid,xtestname,xtestdate,xteststarttime,xtestendtime from tblXTest where xtestgid='$grpr2[0]' AND xtestdate='$cdate' AND  xteststarttime<='$ctime' AND xtestendtime>='$ctime' AND xtestdate='$cdate' AND  xteststatus=1";
      ...
      echo '<option value="'.$row['xtestid'].'">'.$row['xtestname'].' &nbsp &nbsp '.$day.'-'.$month3.'-'.$year.'   ( '.date("g:i a",strtotime($row['xteststarttime'])).' -- '.date("g:i a",strtotime($row['xtestendtime'])).' ) </option>';
                          }
...
<?php
 ...
 if(isset($_POST['submtest_submit']))
 {
  ...
  if($submtestname=="none")
    echo '<script>alert("Please Select a Test Event")</script>';
  else
  {
...
$check="SELECT count(xtest_answr_id)from tblXSubmtest where xsubmtest_gid='$grpr2[0]' AND xsubmtest_gstudid='$sql1row[0]' AND xsubm_testid='$submtestname' AND xtest_answr_status IN(SELECT xsubm_testid where xtest_answr_status=1)";
$checkresult=$conn->query($check);
$checkrow=mysqli_fetch_array($checkresult);

//Returns count of selected testrecord which is previously submitted and status=-1 
$check2="SELECT count(xtest_answr_id)from tblXSubmtest where xsubmtest_gid='$grpr2[0]' AND xsubmtest_gstudid='$sql1row[0]' AND xsubm_testid='$submtestname' AND xtest_answr_status=-1";
...
if($checkrow2[0]>0)
{
  ...
  $sql2="UPDATE tblXSubmtest set xtest_answr='$file',xtest_answr_path='$filepath',xtest_answr_status=1 where xsubmtest_gid='$grpr2[0]' AND xsubmtest_gstudid='$sql1row[0]' AND xsubm_testid='$submtestname' AND xtest_answr_status=-1";
...
else if($checkrow[0]>0)//previously submittes & status=1
{
  ...
  $sql2="UPDATE tblXSubmtest set xtest_answr='$file',xtest_answr_path='$filepath',xtest_answr_status=1 where xsubmtest_gid='$grpr2[0]' AND xsubmtest_gstudid='$sql1row[0]' AND xsubm_testid='$submtestname'";
  if($conn->query($sql2)===TRUE)
  {
...
{
$sql2="INSERT INTO tblXSubmtest(xsubmtest_gid,xsubmtest_gstudid,xsubm_testid,xtest_answr,xtest_answr_path,xtest_answr_status) values('$grpr2[0]','$sql1row[0]','$submtestname','$file','$filepath',1)";
  ...
  else
  {
    ...
    echo '<script>location.href="xstudsubmittest.php?gname='.$gname.'"</script>';
  }
